...
uint256 hashGenesisBlock("0xe14c4b3e221aeb527fbe67bf23ebf64253cda27239b0f575ca97687ba9a00cd2");
...
const string strMessageMagic = "Coursecoin Signed Message:\n";
...
{
    // Coursecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Coursecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Coursecoin: 3.5 days
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Coursecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Coursecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf24cbf35a6ba301ff47ee8c2533762f89c692abfe1c0d80b53dcdc2e9365c9a4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "22-6-2023 - bxb town : the hood";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("045bb927930fa47d722a6b29796df57c79891e8b152c68bb447e2771b8289ce845dab6c4df7c4f63c7ea23209cb28de04fb2d638ff21f73ccdfd095899f3e6c3c2") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1687435821;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084524493;

        if (fTestNet)
        {
            block.nTime    = 1687435892;
            block.nNonce   = 2085279690;
        }
...
block.GetHash().ToString().c_str());
        ...
        assert(block.hashMerkleRoot == uint256("0xd60fda9f404077f96653c248a4e143553833ee4dbff5dd761900431f96fc4929"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xcb, 0xb6, 0xd9 }; // Coursecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CoursecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CoursecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CoursecoinMiner(CWallet *pwallet)
{
    printf("CoursecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("coursecoin-miner");
    ...
    try { loop {
        ...
        printf("Running CoursecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CoursecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CoursecoinMiner, pwallet));
}
